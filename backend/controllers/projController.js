...
const setGradProj = asyncHandler(async (req, res) => {
  ...
  if (req.files) {
    ...
    req.files.forEach((file) => {
      ...
      const url = `https://gyarbstorage.blob.core.windows.net/images/${file.originalname}/?sv=2021-06-08&ss=bfqt&srt=sco&sp=rwdlacupiytfx&se=2023-01-01T08:36:38Z&st=2022-12-18T00:36:38Z&sip=81.227.69.179&spr=https,http&sig=aTYVYZFiNOzHmHLQ0GGDLKbAGnOQeta7HrWd7jb7jL8%3D`; //TODO : hide url in env
      images.push(url);
      console.log(file); //TODO : add a foreach loop to put each file
      //fetch with url and SAS token
      fetch(url, {
        method: "PUT",
        headers: { "x-ms-blob-type": "BlockBlob" }, //TODO : add header for wich content type it is, can base it on the ending of original name
